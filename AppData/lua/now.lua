--[[
date_translator: 将 `now` 翻译为当前日期和时间
   ...
   if (input == "now" or input == "xz") then
      --[[ 用 `yield` 产生一个候选项
           候选项的构造函数是 `Candidate`，它有五个参数：
            ...
            - comment: 候选项的注释
       --]]
      yield(Candidate("now", seg.start, seg._end, os.date("%Y-%m-%d %H:%M:%S"), " 当前时间短格式"))

      --[[ 用 `yield` 再产生一个候选项
           最终的效果是输入法候选框中出现两个格式不同的当前日期的候选项。
      --]]
      yield(Candidate("now", seg.start, seg._end, os.date("%Y年%m月%d日 %H:%M:%S"), " 当前时间长格式"))
   end
end
