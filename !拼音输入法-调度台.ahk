...
;添加一级菜单
; Menu,tray,DeleteAll
Menu, tray, Add, %RimeTitle% 便携版（%AppVersion%版）, MenuHandler
...
Menu, MySubmenu1, Add, 1.3、设置【%AppTitle%】跟随系统启动, SetAutoRunWithMsgHandler
...
menu, MySubmenu1, add ; 分隔符
Menu, MySubmenu1, Add, 1.7、单独安装【%RimeTitle%】, InstallHandler
Menu, MySubmenu1, Add, 1.8、单独卸载现有【%RimeTitle%】, UninstallHandler
Menu, tray, Add, 1、安装相关, :MySubmenu1

;添加项目到子菜单MySubmenu2，格式和上面一样
Menu, MySubmenu2, Add, 2.1、TsfTools配置工具（大名鼎鼎的Win10输入法配置工具，去除烦人的英文输入法）, TSFToolHandler
Menu, MySubmenu2, Add, 2.2、RimeControl配置工具（第三方配置工具，可以独立设置TSF功能是否生效）, RimeControlHandler
Menu, MySubmenu2, Add, 2.3、ImeTool配置工具（Win7的输入法配置工具）, ImeToolHandler
menu, MySubmenu2, add ; 分隔符
Menu, MySubmenu2, Add, 2.4、【%RimeTitle%】自带功能..., MenuHandler
menu, MySubmenu2, add ; 分隔符
Menu, MySubmenu2, Add, 2.5、检查新版本 , RimeUpdateHandler
Menu, MySubmenu2, Add, 2.6、一键卸载【%RimeTitle%】, UninstallAllHandler
menu, MySubmenu2, add ; 分隔符
Menu, MySubmenu2, Add, 2.7、配置修改后，重新部署, RimeDeployHandler
Menu, tray, Add, 2、输入法日常配置, :MySubmenu2

Menu, MySubmenu2_3, Add,2.3.1、说明书 , RimeReadmeHandler
menu, MySubmenu2_3, add ; 分隔符
Menu, MySubmenu2_3, Add,2.3.2、输入法设定 , RimeSettingHandler
Menu, MySubmenu2_3, Add,2.3.3、用户词典管理 , RimeBackupHandler
Menu, MySubmenu2_3, Add,2.3.4、用户资料同步 , RimeSyncHandler
Menu, MySubmenu2_3, Add,2.3.5、重新部署 , RimeDeployHandler
menu, MySubmenu2_3, add ; 分隔符
Menu, MySubmenu2_3, Add,2.3.6、算法服务 , RimeWeaselServerHandler
menu, MySubmenu2_3, add ; 分隔符
Menu, MySubmenu2_3, Add,2.3.7、用户文件夹 , RimeUserDirHandler
Menu, MySubmenu2_3, Add,2.3.8、程序文件夹 , RimeWeaselDirHandler
menu, MySubmenu2_3, add ; 分隔符
Menu, MySubmenu2_3, Add,2.3.9、检查新版本 , RimeUpdateHandler
menu, MySubmenu2_3, add ; 分隔符
Menu, MySubmenu2_3, Add,2.3.10、安装选项 , RimeWeaselSetupHandler
Menu, MySubmenu2_3, Add,2.3.11、卸载输入法 , UninstallAllHandler
Menu, MySubmenu2, Add, 2.4、【%RimeTitle%】自带功能..., :MySubmenu2_3

;添加项目到子菜单MySubmenu2，格式和上面一样
Menu, MySubmenu3, Add, 3.1、一键生成八股文词库，并生效（自定义词库修改后必须执行）, CreateEssayHandler
menu, MySubmenu3, add ; 分隔符
Menu, MySubmenu3, Add, 3.2、生成多音字自定义词库（用于人名匹配）, CreatePinyinDuoYinZhiHandler
menu, MySubmenu3, add ; 分隔符
...
If IsWindows10()
{
    ...
    menu, MySubmenu2, Disable, 2.3、ImeTool配置工具（Win7的输入法配置工具）
}
else
{
    ; 如果不是Win10，以下菜单屏蔽
    menu, tray, Disable, 【经典Ctrl加空格切换IME】
    menu, MySubmenu1, Disable, 1.5、添加简体中文美式键盘
    menu, MySubmenu1, Disable, 1.6、设置【简体中文美式键盘】为默认输入法
}

; 按键定义必须放在最后，否则会导致定义后的代码失效
...
IsWindows10()
{
    return InStr(A_OSVersion,"10.")==1 ? True : False
}
...
StartupDelayerHandler:
    hint = 请务必设置【WeaselServer】（小狼毫算法服务）为延期启动（延期几秒即可）！`n具体操作如下：`n1、在【正常自启动】列表中，选择【WeaselServer】；`n2、点击右键弹出菜单选择【延迟已选择的程序】。
    ShowTip(RimeTitle,hint)
    MsgBox %hint%
...
; 1.6、设置【简体中文美式键盘】为默认输入法
SetDefeatImeHandler:
    hint = 1、选择【美式键盘】，点击【启动】按钮。`n2、【美式键盘】出现在【启动的输入法】列表中时；`n3、 右键点击 【美式键盘】弹出菜单；`n4、点击【设置为默认输入法】！
...
TSFToolHandler:
    ...
    {
        MsgBox Win10输入法配置工具只支持Win10，Win7版本请使用ImeTool。
    }
return

; 3、ImeTool配置工具（Win7的输入法配置工具）
ImeToolHandler:
    If IsWindows10()
    {
        MsgBox % "Win10环境请尽量使用TsfTools来代替ImeTool！"
    }

    RunWait *RunAs ImeTool@Win7.exe ,%A_WorkingDir%\10-输入法配置工具\
return
;一键生成八股文词库
CreateEssayHandler:
